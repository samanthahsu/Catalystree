...
public class GasDataBaseAdapter {

    private static final String DATABASE_NAME = "catalystree.db";
    private static final int DATABASE_VERSION = 1;
    public static final int NAME_COLUMN = 1;
    private static final String DATABASE_TABLE ="GAS";
    ...
    public GasDataBaseAdapter(Context _context) {
        context = _context;
        dbHelper = new MasterDataBaseHelper(context);
    }

    public GasDataBaseAdapter open() throws SQLException {
    ...
    public void insertEntry(String username, String date, int cost) {
        ...
        db.insert("GAS", null, newValues);
        Toast.makeText(context, "Data Saved", Toast.LENGTH_SHORT).show();
    }
    public int deleteEntry(String username) {
        //String id=String.valueOf(ID);
        String where = "ID=?";
        int numberOFEntriesDeleted = db.delete("GAS", where, new String[]{username});
        // Toast.makeText(context, "Number fo Entry Deleted Successfully : "+numberOFEntriesDeleted, Toast.LENGTH_LONG).show();
        return numberOFEntriesDeleted;
    }

    public Cursor getGasEntry(String username) {

        Cursor c = db.query(true, "GAS", new String[]{"date, cost"} ,"USERNAME=?", new String[]{username}, null, null, null, "12");
        if (c.getCount() < 1) {
            c.close();
        }
        for (int i = 0; i < 12 ; i++) {
